// 浏览器内冒烟测试：真的去点卡、结束回合、走地图，并检查 BGM 有没有跟着场景切换。
// 用法: node tools/smoke.mjs
//
// 注意：这个脚本会把内嵌的测试代码写到 tools/smoke-script.js，页面用 ?smoke=1 加载它。
// 所以要改测试内容，改这里的 SCRIPT 字符串（不要直接改 tools/smoke-script.js，会被覆盖）。
 ...
 * ⚠ 写盘之前先查一遍模板有没有被**提前闭合**。
 *
 * 上面那个 SCRIPT 是一整块模板字符串，而它里面全是**注释和中文** ——
 * 只要哪条注释里手写了一个反引号（比如「`.fx-projectile`」这种写法），
 * 模板就在那儿结束了、后面半截被当成真正的代码：
 * `node --check` **照样通过**（语法完全合法），跑起来才报一句
 * 「ReferenceError: xxx is not defined」，而且报的位置和真正的问题差了几百行，
 * 找起来很费劲（这个坑前后踩了五次）。
 * 所以这里用两个「只在脚本尾部出现」的标记当哨兵：缺了就说明被截断了。
 */
for (const sentinel of ['SMOKE_OK', "log('ERRORS=' + JSON.stringify(errors))"]) {
  if (!SCRIPT.includes(sentinel)) {
    console.error(`✗ 冒烟脚本模板被截断了（找不到 ${sentinel}）——`
      + '十有八九是上面某条注释里写了反引号或 ${，把模板字符串提前闭合了；'
      + '注释里要引用代码请用「」引号。');
    process.exit(1);
  }
}

await fs.writeFile(path.join(ROOT, 'tools', 'smoke-script.js'), SCRIPT, 'utf8');

const URL_ = 'http://127.0.0.1:5123/?smoke=1';
const args = [
  '--headless=new',
  '--disable-gpu',

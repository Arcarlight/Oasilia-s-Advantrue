...
const has = (c, pred) => (c.effects ?? []).some(pred);
...
for (const c of cards) {
  ...
  // ---- 多段攻击：文案「连续 N 次」必须真的打 N 下 ----
  const multi = t.match(/连续\s*(\d+)\s*次/);
  if (multi) {
    const want = num(multi[1]);
    const got = (c.effects ?? []).filter((e) => e.kind === 'damage').reduce((m, e) => Math.max(m, e.hits ?? 1), 0);
    add(c, `连续 ${want} 次`, got === want, `damage.hits = ${got}（应为 ${want}）`);
  } else if ((c.effects ?? []).some((e) => e.kind === 'damage' && (e.hits ?? 1) > 1)) {
    add(c, '多段（文案未标注）', false, `effects 有 hits=${(c.effects ?? []).find((e) => e.hits > 1).hits} 但文案没写「连续 N 次」`);
  }
  ...
  if (pierceAll || pierceHalf) {
    ...
    const dmg = (c.effects ?? []).filter((e) => e.kind === 'damage');
  ...
  if (exec) {
    ...
    const ok = (c.effects ?? []).some((e) => e.kind === 'damage' && Math.abs((e.execThreshold ?? -1) - want) < 1e-6);
  ...
  if (draw) {
    ...
    const got = (c.effects ?? []).filter((e) => e.kind === 'draw').reduce((s, e) => s + e.n, 0);
  ...
  if (ap) {
    const want = num(ap[1]);
    const got = (c.effects ?? []).filter((e) => e.kind === 'ap').reduce((s, e) => s + (e.n ?? 0), 0);
  ...
  for (const m of t.matchAll(/(攻击|防御|敏捷|幸运)\s*([+\-])(\d+)(%)?/g)) {
    ...
    const got = (c.effects ?? []).filter((e) => e.kind === 'buff' && e.stat === key)

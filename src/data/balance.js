...
export const BALANCE = {
   ...
   *
   * 3.0.2 从 200 收到 80：实测单张「本场战斗威力 +N%」的牌（血祭 +25 / 背水一战 +45）
   * 在 6 回合里的贡献本来就只有 10~20%（见 tools/probe-power-cards.mjs），
   * 上限 200 等于允许「叠到三倍伤害」，那是另一回事。
   *
   * 3.1.10 收到 65，同时把三张牌本身削了（血祭 25→18 / 背水一战 45→30 / 过热 35→25）：
   * 玩家反馈「类似血祭这种卡百分比加威力的还是有些偏强，只要用两次就能叠到 80% 的增加威力」。
   * 现在要**三张**才够得到 65 的上限（18+30+25），而这三张都是「使用后销毁」的一次性牌。
   */
  strengthCap: 65,
  ...
  /**
   * **敌人的同一个回合最多打掉玩家最大生命的这个比例**（3.0.1 加的兜底）。
   *
   * 敌人的出牌数**不封顶**（用户要求：封顶之后敌人太软、没有火候），所以「对手抽得极顺」的那一回合
   * 仍然能打到玩家最大生命的 70~89% —— 于是「满血进场、一回合见底」偶尔还会发生
   * （用户报的正是这件事）。0.7 = 从七成血以上进场，**一定**能活过对手这一回合。
   *
   * 只限「敌人的回合」：玩家自己的失误（残血开怪、不带药）该照常受罚；
   * 而「看不见对手手牌、被一波带走」没有对策空间，那是设计问题不是难度。
   * 玩家身上的持续伤害在自己回合开始结算，不受这条限制。
   */
  enemyTurnDamageCapPct: 0.7,
 ...
 * 普通战斗**连续几场不掉卡**就要保底。
 *
 * 玩家报的：「怎么现在会出现连续好几场都不掉卡牌的情况？」
 * 查下来不是 bug、也不是「抽到了不给玩家的牌」（那个池子根本不收 enemyOnly 的牌，
 * 实测 2 万次抽卡一张都没漏），就是 `cardRewardChance` 这个 30% 的骰子在连空：
 * 实测连空 3 场占所有空手段的 5.6%、最长能连空 5 场，**大约每三局就会撞到一次 3 连空**。
 * 单看概率没问题，但「连着好几场什么都没有」的体感很差。
 *
 * 所以加一条保底：**连空 2 场之后，下一场必出卡**。
 * 2 万局模拟（每局 24 场普通战斗）：空手率只从 30.0% 涨到 28.6%（几乎不动），
 * 但**3 连空从此不可能出现**。
 * 精英 / 首领本来就是必出，也顺手把这个计数清零。
 */
cardDroughtPity: 2,
  /**

...
export class Battle {
   ...
   * 从牌堆顶取一张**本回合还能用**的牌。
   *
   * 「本回合打出去的牌」躺在牌堆底部并带着 playedTurn 标记，这里会跳过它们
   * （放回底部，继续往下翻）。取不到就返回 null，交给调用方决定要不要洗弃牌堆。
   * 翻牌次数最多一轮牌堆长度，所以不会死循环。
   */
  takeFromDraw(key) {
    ...
    let card = null;
    while (d.draw.length) {
      const cand = d.draw.shift();
      if (cand.playedTurn === this.turn) { held.push(cand); continue; }
      card = cand;
      break;
    }
    // 翻过去的牌原样放回底部：它们只是「这一回合不能再被抽到」，不是消失了
    for (const h of held) d.draw.push(h);
    return card;
  }
   ...
   *
   * 注意「本回合打出去的牌不能再被抽回来」这条规则（见 takeFromDraw）：
   * 没有它的话，一副只有「子弹拳 + 电光一闪」的卡组能两张牌来回刷，
   * 每回合都把出牌上限打满、把对手直接揍死（用户反馈的「无限循环」）。
   */
  drawCards(key, n) {
    const d = this.decks[key];
    ...
    const drawn = [];
    for (let i = 0; i < n; i++) {
      let card = this.takeFromDraw(key);
      if (!card) {
        if (d.discard.length === 0) break;
        // 洗牌：弃牌堆混进来，本回合打过的那些仍然留在底部（还带着标记）
        d.draw = [...d.draw, ...this.rng.shuffle(d.discard)];
        d.discard = [];
        this.emit({ type: 'reshuffle', side: key });
        card = this.takeFromDraw(key);
        if (!card) break;   // 洗完还是只剩本回合打过的牌 —— 那就是抽不到
      }
    ...
    if (drawn.length) this.emit({ type: 'draw', side: key, cards: drawn.map((c) => c.id) });
      ...
      // 一张都没抽到：如果是因为「这回合刚打出去的牌还在牌堆底部歇着」，就说一句 ——
      // 不然玩家只看到「抽 2 张」什么也没发生，会以为是 bug。
      const resting = d.draw.filter((c) => c.playedTurn === this.turn).length;
      if (resting > 0) {
        this.emitLogged(
          { type: 'drawWhiff', side: key, resting },
          `想抽牌，但这一回合打出去的 ${resting} 张牌要下回合才回到牌堆——没抽到。`,
          'note',
        );
      }
    }
  ...
  shuffleAll(key) {
    ...
    d.discard = [];
    d.hand = [];
    // 「本回合打过」的标记也要清掉：战斗之间不该互相影响
    for (const c of d.draw) delete c.playedTurn;
  }
  ...
  playCard(uid, opts = {}) {
    ...
    // 使用后的去向：销毁区 or 卡组最底端
    // 回牌堆的牌会打上 playedTurn 标记：**这一回合不会再被抽回来**（下回合自动作废）。
    // 这是「0 费抽 1 张」类卡牌的天花板 —— 否则小卡组能靠两张牌无限轮换刷死对手。
    if (card.exhaust) {
      d.exhaust.push(entry);
      this.emit({ type: 'exhaust', side: 'player', id: card.id });
    } else {
      entry.playedTurn = this.turn;
      d.draw.push(entry);
  ...
  enemyAct() {
    ...
    while (!this.over && guard++ < 14) {
      ...
      if (pick.c.card.exhaust) {
        ...
        // 同上：这一回合不会再被抽回来（敌人的 0 费抽牌也刷不起来）
        pick.c.playedTurn = this.turn;
  ...
  snapshot(key) {
    ...
    return {
      ...
      // 牌堆里「本回合刚打出去、暂时抽不到」的张数（诊断用）
      restingCount: this.decks[key].draw.filter((c) => c.playedTurn === this.turn).length,
    };
  }

...
* **纯函数里不许掷骰子**（v2.7 修的真 bug）：`upgradeCandidates()` 的打分里带着 `this.rng() * 0.9`，
  于是**每调用一次名单就重掷一次** —— 界面摆出来的三张是第一次算的，玩家点下去之后
  `restUpgrade` 又算了一遍，挑中的那张不在新名单里就走了兜底 `?? cands[0]`：
  用户看到的就是「冥想给了三个选项，点哪张都是随机给一张」（实测 360 次里给错 329 次）。
  现在的做派：**界面能显示出来的东西，引擎必须能用同一份输入重新算出一模一样的一份**
  （用 `stableJitter(原卡 id + 候选 id)` 代替 rng），而且 `restUpgrade` 只认「玩家挑的那张」
  （真对不上就如实拒绝，绝不偷偷换一张）。测试是用 `cands[0]` 写的 —— 那恰好永远能对上，
  所以旧测试全绿也抓不到；现在**三张各点一遍**并断言给的就是那张。
* **掉落道具是「发了再说」，不是「点了才发」**：`finishBattle` 里就 `grantDrop()`（进 held + 存档），
  界面只负责展示结果（`reward.itemDrop = { stored / overflow }`）。顺带钉住一条顺序：
  **`bossKills + 1` 必须在发掉落之前**（原来写在 takeRewardCard，首领掉的东西按扩容前的上限判定，
  玩家就撞上「背包已经扩充，但还是提示拿不下」）。
* 「捡到道具」那一屏**不是新 phase**：它挂在 `phase === 'reward'` 里，靠 `reward.itemDrop.seen` 区分，
  点「收下，去结算」把它标记掉就换屏。这样阶段机、存档、所有现有断言都不用动 ——
  新开一个 phase 会牵动 settle / resume / 一大堆 `phase === 'reward'` 的判断，不值当。
* **浅色羊皮纸面板上别用 `--accent-2`**（亮金是给深色背景调的）：`.panel-title` 的注释里早就记着，
...
* **卡池去重是「内容问题」，不是平衡问题**（v2.9）：用户第三次提卡池污染，这次的点是
  「啄和撞击完全是一样的效果」。查下来玩家池 233 张里藏着 **35 组「同费用 + effects
  逐字节相同」的重复**（48 张多余）—— 类型在这游戏里只是外观（battle.js 里没有任何属性倍率，
  敌人属性只用于掉落加权），所以换个名字的同一张牌在玩法上就是同一张牌。
  处理：`tools/dedupe-pool-cards.mjs`（新增，默认 dry run，`--write` 才落盘）每组留一张、
  其余转 `enemyOnly`。**留哪张的三条优先级**：起始卡组 > 不在任何招式池的（留在池里才不会
  变成死卡）> 稀有度低的。玩家池 233 → 185 张。两条新门禁（都在 check-content）：
  ① 玩家池里不许再有「同费用 + 完全同效果」的重复；② `enemyOnly` 的牌**必须真的有人会用**
  （在某个招式池或某只敌人的专属招里），否则图鉴永远点不亮 —— 它的解锁条件是「看见敌方打出」。
  顺带一个教训：想把重复的那张补给敌人招式池时被门禁拦了（「0 费还抽牌」的卡不能进敌人池），
  于是改成给它换个身份（子弹拳：抽 1 → 无视 30% 防御）。**给卡换个身份往往比硬塞池子干净。**
* **改了效果数值就要回头改卡面文案**（v2.3 埋的雷，v2.9 才拆）：那一版把无视防御抬高了
  （30%→50%、50%→70%）却忘了卡面印的数字，11 张牌的卡面比实际低一档（燕返写 30%、跑 50%）。
  现在 check-content 会逐张核对卡面「无视对手 X% 防御」与 `ignoreDefPct`（全部=100%、一半=50%）。
  同类隐患：任何「文案里带数字」的效果（层数、回合数、百分比）都该配一条门禁。
